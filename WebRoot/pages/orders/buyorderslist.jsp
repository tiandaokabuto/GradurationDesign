...
<tr>
<th colspan="4">
订单编号：${ordersExtend.oid }　成交时间：${ordersExtend.otime }　金额：<font color="red"><b>${ordersExtend.total }</b></font>　
		<c:choose>
			<c:when test="${ordersExtend.state eq 1 }">
				等待发货
			</c:when>
			<c:when test="${ordersExtend.state eq 2 }">
				<a href="<c:url value='/confirmorders.action?oid=${ordersExtend.oid }'/>">确认收货</a>
			</c:when>
			<c:when test="${ordersExtend.state eq 3 }">
				交易成功
			</c:when>			
		</c:choose>
</th>
  ...
  <tbody>
	...
	<tr>
		...
		<td>商品名:${orderitemExtend.goodsExtend.gname }</td>
		<td>单价:${orderitemExtend.goodsExtend.newprice }</td>
		<td>小计:${orderitemExtend.subtotal }</td>
	</tr>
	</c:forEach>
	</c:forEach>
  </tbody>
